...
> 你确信这就是正确的密码，但门却打不开。你敲了敲门，没人回应。你一边思考，一边堆了个雪人。

As you're rolling the snowballs for your snowman, you find another security document that must have fallen into the snow:

> 在滚雪球堆雪人的时候，你发现了另一份安全文件，它一定是掉进了雪里：

"Due to newer security protocols, please use **password method 0x434C49434B** until further notice."

> "由于更新的安全协议，请使用**密码方法 0x434C49434B**，直至另行通知。"

You remember from the training seminar that "method 0x434C49434B" means you're actually supposed to count the number of times **any click** causes the dial to point at `0`, regardless of whether it happens during a rotation or at the end of one.

> 你从培训研讨会中记得，"方法 0x434C49434B" 意味着实际上要统计**任意一次咔嗒声**使转盘指向 `0` 的次数，无论这发生在旋转过程中还是旋转结束时。

Following the same rotations as in the above example, the dial points at zero a few extra times during its rotations:

> 按照上述示例中的相同旋转指令，转盘在旋转过程中多指向了几次零：
...
> - 转盘初始指向 `50`。
> - 旋转 `L68` 指向 82；在此过程中，转盘指向 `0` **一次**。
> - 旋转 `L30` 指向 `52`。
> - 旋转 `R48` 指向 **`0`**。
> - 旋转 `L5` 指向 `95`。
> - 旋转 `R60` 指向 `55`；在此过程中，转盘指向 `0` **一次**。
> - 旋转 `L55` 指向 **`0`**。
> - 旋转 `L1` 指向 `99`。
> - 旋转 `L99` 指向 **`0`**。
> - 旋转 `R14` 指向 `14`。
> - 旋转 `L82` 指向 `32`；在此过程中，转盘指向 `0` **一次**。

In this example, the dial points at `0` three times at the end of a rotation, plus three more times during a rotation. So, in this example, the new password would be **`6`**.

> 在此示例中，转盘在旋转结束时指向 `0` 三次，再加上旋转过程中指向 `0` 三次。因此，此示例中的新密码是 **`6`**。

Be careful: if the dial were pointing at `50`, a single rotation like `R1000` would cause the dial to point at `0` ten times before returning back to `50`!

> 注意：如果转盘指向 `50`，一次 `R1000` 的旋转会使转盘在回到 `50` 之前十次指向 `0`！

Using password method 0x434C49434B, **what is the password to open the door?**

> 使用密码方法 0x434C49434B，**打开门的密码是多少？**
